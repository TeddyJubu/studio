...
const SentimentAnalysisOutput = z.object({
  ...
  sentimentScore: z.number().min(-1).max(1).describe('Score from -1 (very negative) to 1 (very positive)'),
  ...
  urgency: z.enum(['low', 'medium', 'high', 'critical']).describe('How urgently this needs attention'),
  needsEscalation: z.boolean().describe('Whether this should be escalated to a human'),
  escalationReason: z.string().optional().describe('Why escalation is needed'),
  keyPhrases: z.array(z.string()).describe('Key phrases that influenced sentiment analysis'),
  tone: z.enum(['formal', 'casual', 'professional', 'aggressive', 'friendly', 'sarcastic']),
  confidence: z.number().min(0).max(1).describe('Confidence in sentiment analysis'),
});
